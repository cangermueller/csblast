...
#ifndef SRC_CSSAMPLE_H_
#define SRC_CSSAMPLE_H_
...
std::ostream& usage(const Params& params, std::ostream& out = std::cout) {
  ...
  out << strprintf("  %-30s %s\n", "-W, --window-length [0,inf[",
                   "Sample context profiles of length W instead of full-length "
                   "profiles");
...
void sample_profiles(
    const Params& params,
    const std::vector< shared_ptr< CountProfile<Alphabet> > >& profiles,
    std::vector< shared_ptr< CountProfile<Alphabet> > >& samples,
    std::ostream& out) {
  typedef std::vector< shared_ptr< CountProfile<Alphabet> > > counts_vector;
  typedef typename counts_vector::const_iterator profile_iterator;
  ...
  for (profile_iterator it = profiles.begin(); it != profiles.end()
         && static_cast<int>(samples.size()) < params.sample_size; ++it) {
    if (params.window_length == 0) {  // add full length profile to samples
      samples.push_back(*it);

    } else if ((*it)->num_cols() >= params.window_length) {
      // sample context windows if profile has sufficient length
      std::vector<int> idx;  // sample of indices
      for (int j = 0; j <= (*it)->num_cols() - params.window_length; ++j)
        idx.push_back(j);
      ...
      for (index_iterator ii = idx.begin(); ii != idx.end()
             && static_cast<int>(samples.size()) < params.sample_size; ++ii) {
        shared_ptr< CountProfile<Alphabet> > p(
            new CountProfile<Alphabet>(**it, *ii, params.window_length));
        samples.push_back(p);
      }
    }
  }
...
void cssample(const Params& params, std::ostream& out = std::cout) {
  typedef std::vector< shared_ptr< CountProfile<Alphabet> > > profiles_vector;
  typedef typename profiles_vector::const_iterator profile_iterator;

  // large pool of profiles to sample from
  std::vector< shared_ptr< CountProfile<Alphabet> > > profiles;
  // sampled profiles
  std::vector< shared_ptr< CountProfile<Alphabet> > > samples;
  ...
  if (!fin)
    throw Exception("Unable to read from input file '%s'!",
                    params.infile.c_str());
  out << strprintf("Reading profiles from %s ...",
                   get_file_basename(params.infile).c_str());
  out.flush();
  LOG(INFO) << strprintf("Reading profiles from %s ...",
                         get_file_basename(params.infile).c_str());
  CountProfile<Alphabet>::readall(fin, profiles);
  ...
  if (!fout) throw Exception("Unable to write profiles to output file '%s'!",
  ...
  LOG(INFO) << strprintf("Wrote %i profiles with a total number of %i "
                         "columns to %s",
                         samples.size(), num_cols, params.outfile.c_str());
  fout.close();
}
