    ...
    for (size_t i = 0; i < profile_.length(); ++i) {
        ...
        if (!hide_numbering) {
            if (profile_.length() < 20 && profile_.length() % 2 == 1) {
                ...
                fprintf(fp, "\\node[colidx] at (%zu,0) {\\tiny \\textcolor{%s}{%d}};\n",
                        i, idx == 0 ? "red" : "black", idx);
            } else {
        ...
        for (size_t k = 0; k < tuples.size(); ++k) {
            ...
            fprintf(fp, "\\filldraw[logo,fill=%scol%c] (%.4f,%.4f) rectangle +(1,%.4f);\n",
                    Abc::kName, Abc::kIntToChar[res], x, y, prob * 20);
            if (prob > 0.08)
                fprintf(fp, "\\node at (%zu,%.4f) {\\tiny \\bf \\sffamily %c};\n",
                        i, y + prob * 10, Abc::kIntToChar[res]);
            y += prob * 20;
        }
    }
