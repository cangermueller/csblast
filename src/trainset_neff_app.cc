...
struct TrainsetNeffAppOptions {
  static const int kAssignMatchColsByQuery = -1;

  TrainsetNeffAppOptions() { Init(); }
  virtual ~TrainsetNeffAppOptions() {}
...
};  // TrainsetNeffAppOptions


template<class Abc>
class TrainsetNeffApp : public Application {
 private:
  ...
  // Parameter wrapper
  TrainsetNeffAppOptions opts_;
};  // class TrainsetNeffApp



template<class Abc>
void TrainsetNeffApp<Abc>::ParseOptions(GetOpt_pp& ops) {
...
template<class Abc>
void TrainsetNeffApp<Abc>::PrintBanner() const {
  fputs("Calculates Neff in training profiles of a training set.\n", out_);
}

template<class Abc>
void TrainsetNeffApp<Abc>::PrintUsage() const {
  fputs("Usage: trainset_neff -i <infile> [options]\n", out_);
}

template<class Abc>
void TrainsetNeffApp<Abc>::PrintOptions() const {
  fprintf(out_, "  %-30s %s\n", "-i, --infile <file>",
          "Input file with a training set");
  fprintf(out_, "  %-30s %s\n", "-x, --outfile-x <file>",
...
template<class Abc>
inline void TrainsetNeffApp<Abc>::Write(FILE* fout, double neff) const {
...
int main(int argc, char* argv[]) {
  ...
  if (alphabet == "dna" || alphabet == "DNA")
    return cs::TrainsetNeffApp<cs::Dna>().main(argc, argv, stdout, "trainset_neff");
  else
    return cs::TrainsetNeffApp<cs::AA>().main(argc, argv, stdout, "trainset_neff");
}
