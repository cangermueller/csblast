...
#ifndef SRC_CHAIN_GRAPH_H_
#define SRC_CHAIN_GRAPH_H_
...
class ChainGraph {
 public:
  ...
  friend std::ostream& operator<< (std::ostream& out, const ChainGraph& g) {
    g.Print(out);
    return out;
  }
...
class TransitionAdaptor {
 public:
  TransitionAdaptor(ChainGraph<Alphabet, State>* g, int k, int l)
      : g_(g), k_(k), l_(l) {}
...
template< class Alphabet, template<class> class State >
void NormalizeTransitions(ChainGraph<Alphabet, State>* graph, float f = 1.0f);
...
class StateInitializer {
 public:
  ...
  virtual void Init(ChainGraph<Alphabet, State>& graph) const = 0;
};  // class StateInitializer
