...
#ifndef SRC_CSBUILD_H_
#define SRC_CSBUILD_H_
...
std::ostream& usage(const Params& params, std::ostream& out = std::cout) {
  ...
  out << strprintf("  %-30s %s (def=%s)\n", "-f, --format <string>",
                   "Input data format: seq, fas, a2m, or a3m",
                   params.format.c_str());
  out << strprintf("  %-30s %s\n", "-M, --matchcol [0:100]",
...
void csbuild(const Params& params, std::ostream& out = std::cout) {
  ...
  if (params.format == "seq") {
    ...
    typename Alignment<Alphabet_T>::Format f =
      alignment_format_from_string<Alphabet_T>(params.format);
