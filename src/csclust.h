...
#ifndef SRC_CSCLUST_H_
#define SRC_CSCLUST_H_
...
void Params::parse_options(GetOpt_pp& options) {
  ...
  if (outfile.empty())
    outfile = directory + get_file_basename(infile, false) + "lib";
}
...
std::ostream& usage(const Params& params, std::ostream& out = std::cout) {
  ...
  out << strprintf("  %-30s %s (def=%3.1f)\n", "    --lib-pc [0,1]",
                   "Pseudocounts for library profiles", params.lib_pseudocounts);
...
void csclust(const Params& params, std::ostream& out = std::cout) {
  ...
  shared_ptr< ProfileLibrary<Alphabet> > lib_ptr;
  ...
  LOG(INFO) << strprintf("Reading training profiles from %s ...",
                         get_file_basename(params.infile).c_str());
  CountProfile<Alphabet>::readall(fin, data);
  ...
  if (params.libfile.empty()) {
    out << strprintf("Initializing profile library by sampling %i profiles "
                     "from training profiles ...", params.num_profiles);
    out.flush();
    LOG(INFO) << strprintf("Initializing HMM by sampling %i profiles from "
                           "training profiles ...", params.num_profiles);
    SamplingProfileInitializer<Alphabet> profile_init(data,
                                                      &sm_pc,
                                                      params.lib_pseudocounts);
    lib_ptr = shared_ptr< ProfileLibrary<Alphabet> >(
        new ProfileLibrary<Alphabet>(params.num_profiles,
                                     data[0]->num_cols(),
                                     profile_init));
    ...
    out << strprintf("Reading profile library from %s ...",
                     get_file_basename(params.libfile).c_str());
    out.flush();
    LOG(INFO) << strprintf("Reading profile library from %s ...",
                           get_file_basename(params.libfile).c_str());
    lib_ptr = shared_ptr< ProfileLibrary<Alphabet> >(
        new ProfileLibrary<Alphabet>(fin));
    out << std::endl;
  }
  ...
  out << strprintf("Clustering training data by expectation maximization "
                   "(K=%i, W=%i, N=%i) ...", lib_ptr->num_profiles(),
                   lib_ptr->num_cols(), data.size());
  out.flush();
  LOG(INFO) << strprintf("Clustering training data by expectation maximization "
                         "(K=%i, W=%i, N=%i) ...", lib_ptr->num_profiles(),
                         lib_ptr->num_cols(), data.size());
  out << std::endl << std::endl;
  Clustering<Alphabet, CountProfile> em_clust(params, data, *lib_ptr, out);
  em_clust.run();

  // write profile library to outfile
  std::ofstream fout(params.outfile.c_str(), std::ios_base::out);
  if (!fout)
    throw Exception("Unable to write profile library to output file '%s'!",
                    params.outfile.c_str());
  lib_ptr->write(fout);
  fout.close();
  out << std::endl << strprintf("Wrote profile library to %s",
                                params.outfile.c_str()) << std::endl;
  LOG(INFO) << strprintf("Wrote profile_library to %s", params.outfile.c_str());
}

}  // namespace cs

#endif  // SRC_CSCLUST_H_
