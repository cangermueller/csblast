...
TEST(CountProfileTest, AlignmentBpdS) {
  ...
  EXPECT_FLOAT_EQ(1.0f, std::accumulate(profile.col_begin(122),
                                        profile.col_end(122), 0.0f));
}
...
TEST(CountProfileTest, AddMatrixPseudocountsToProfile) {
  ...
  mpc.AddPseudocountsToProfile(DivergenceDependentAdmixture(1.0f, 10.0f), &profile);
